...
public class StudentH2Service implements StudentRepository {
    ...
    @Override
    public Student getStudentById(int stuudentId) {
        try {
            return db.queryForObject("select * from student where studentId = ?", new StudentRowMapper(), stuudentId);
        } catch (Exception e) {
    ...
    @Override
    public Student addSinglStudent(Student student) {
    ...
    public Student updateStudent(int studentId, Student student) {
        ...
        if (student.getStandard() != 0) {
            db.update("update student set stanar = ? where studentId ?", student.getStandard(), studentId);
        }
